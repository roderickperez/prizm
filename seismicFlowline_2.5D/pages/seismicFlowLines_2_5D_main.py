...
LOCAL_APP_DIR = Path(__file__).resolve().parents[1]
...
from core.flowline_algorithms import (
    compute_gst_gpu,
    kmeans_torch,
    rk4_trace_vector_field,
)
from core.flowline_helpers import line_shape_feature, extend_line_to_full_width, build_segment_arrays
...
def setup_flow_logger(log_dir: Path, logger_name: str = "seismic_flowlines_2_5d_app") -> logging.Logger:
    log_dir.mkdir(parents=True, exist_ok=True)
    log_file = log_dir / f"flowlines_2_5d_{time.strftime('%Y%m%d_%H%M%S')}.log"
    ...
    existing_logs = sorted(log_dir.glob("flowlines_2_5d_*.log"), key=lambda p: p.stat().st_mtime)
    ...
    logger.info("FlowLine 2.5D logger initialized")
    return logger
...
class SeismicFlowApp:
    def __init__(self):
        ...
        self.extracted_slices: dict[tuple[str, int], np.ndarray] = {}
        self.extracted_keys_by_mode: dict[str, list[int]] = {"Inline": [], "Crossline": []}
        self.slice_result_cache: dict[tuple[str, int], dict[str, object]] = {}
        self.active_slice_key: tuple[str, int] | None = None
        ...
        self.extract_line_btn = pn.widgets.Button(
            name="Extract 2.5D Lines",
            button_type="default",
            sizing_mode="stretch_width",
            stylesheets=self.button_stylesheets,
        )
        self.extract_mode = pn.widgets.RadioButtonGroup(
        ...
        self.extract_step = pn.widgets.IntInput(
            name="Extract Every Nth Line",
            value=50,
            step=1,
            start=1,
            sizing_mode="stretch_width",
        )
        self.total_lines_info = pn.pane.Markdown(
            "Total Inlines: 0 | Total Crosslines: 0",
            styles={"font-size": "0.9em", "font-weight": "bold"},
        )
        self.view_mode_group = pn.widgets.RadioButtonGroup(
            name="Visualize Extracted Direction",
            options=["Inline"],
            value="Inline",
            button_type="default",
            sizing_mode="stretch_width",
        ...
        self.view_slice_slider = pn.widgets.DiscreteSlider(
            name="Extracted Slice",
            options=[0],
            value=0,
            disabled=True,
            sizing_mode="stretch_width",
        ...
        self.extract_step.stylesheets = self.text_input_stylesheets
        ...
        self.view_mode_group.param.watch(self._on_view_mode_change, "value")
        self.view_slice_slider.param.watch(self._on_view_slice_change, "value")
    ...
    def _state_cache_key(self) -> str:
        ...
        return f"seismic_flowlines_2_5d_state::{cube_part}"

    def _save_state_to_cache(self) -> None:
        try:
            self._save_active_slice_state()
    ...
    def _update_total_lines_info(self) -> None:
        if self.inline_values.size > 0:
            il_text = f"{len(self.inline_values)} ({int(self.inline_values.min())} - {int(self.inline_values.max())})"
        else:
            il_text = "0"
        if self.xline_values.size > 0:
            xl_text = f"{len(self.xline_values)} ({int(self.xline_values.min())} - {int(self.xline_values.max())})"
        else:
            xl_text = "0"
        self.total_lines_info.object = f"Total Inlines: {il_text} | Total Crosslines: {xl_text}"

    def _save_active_slice_state(self) -> None:
        if self.active_slice_key is None:
            return
        self.slice_result_cache[self.active_slice_key] = {
            "locked_meta": dict(self.locked_meta),
            "res_mag": None if self.res_mag is None else np.array(self.res_mag, copy=True),
            "res_ori": None if self.res_ori is None else np.array(self.res_ori, copy=True),
            "res_vec": None if self.res_vec is None else np.array(self.res_vec, copy=True),
    ...
    def _set_active_slice(self, mode: str, idx: int) -> None:
        key = (str(mode), int(idx))
        if key not in self.extracted_slices:
            return

        self._save_active_slice_state()
        self.active_slice_key = key
        self.locked_slice = np.array(self.extracted_slices[key], copy=True)

        cached = self.slice_result_cache.get(key, {})
        self.locked_meta = dict(cached.get("locked_meta", {"mode": mode, "idx": int(idx), "xl": "X", "yl": "Y"}))
        self.locked_meta["mode"] = mode
        self.locked_meta["idx"] = int(idx)
        ...
        has_gst = self.res_mag is not None and self.res_ori is not None
        has_flowlines = len(self.flowlines) > 0
        ...
        self.extract_status.object = f"Active extracted slice: {mode} {idx}"
        self.update_view()

    def _refresh_view_slice_slider(self) -> None:
        mode = str(self.view_mode_group.value)
        values = self.extracted_keys_by_mode.get(mode, [])
        if not values:
            self.view_slice_slider.options = [0]
            self.view_slice_slider.value = 0
            self.view_slice_slider.disabled = True
            return

        self.view_slice_slider.options = [int(v) for v in values]
        self.view_slice_slider.value = int(values[0])
        self.view_slice_slider.disabled = False

    def _on_view_mode_change(self, event=None) -> None:
        self._refresh_view_slice_slider()
        if not self.view_slice_slider.disabled:
            self._set_active_slice(str(self.view_mode_group.value), int(self.view_slice_slider.value))

    def _on_view_slice_change(self, event=None) -> None:
        if self.view_slice_slider.disabled:
            return
        self._set_active_slice(str(self.view_mode_group.value), int(self.view_slice_slider.value))
    ...
    def _sample_axis_values(self, axis_values: np.ndarray, nth: int) -> list[int]:
        if axis_values.size == 0:
            return []
        first = int(axis_values.min())
        last = int(axis_values.max())
        candidates = np.arange(first, last + 1, max(1, int(nth)), dtype=int)
        valid_set = set(axis_values.astype(int).tolist())
        sampled = [int(v) for v in candidates if int(v) in valid_set]
        if first not in sampled:
            sampled.insert(0, first)
        return sampled

    def _ordered_extracted_keys(self) -> list[tuple[str, int]]:
        keys: list[tuple[str, int]] = []
        keys.extend([("Inline", int(v)) for v in self.extracted_keys_by_mode.get("Inline", [])])
        keys.extend([("Crossline", int(v)) for v in self.extracted_keys_by_mode.get("Crossline", [])])
        return keys

    def _line_shape_feature(self, line: np.ndarray, n_samples: int = 64) -> np.ndarray:
        return line_shape_feature(line, n_samples=n_samples)

    def _extend_line_to_full_width(self, line: np.ndarray, width: int, height: int) -> np.ndarray:
        return extend_line_to_full_width(line, width=width, height=height)
    ...
    def run_extraction(self, event) -> None:
        ...
        self.extract_status.object = "Extracting 2.5D lines..."
        try:
            nth = max(1, int(self.extract_step.value))
            mode_selection = str(self.extract_mode.value)

            inline_indices = self._sample_axis_values(self.inline_values, nth=nth)
            xline_indices = self._sample_axis_values(self.xline_values, nth=nth)

            target_keys: list[tuple[str, int]] = []
            if mode_selection in ("Inline", "Both"):
                target_keys.extend([("Inline", idx) for idx in inline_indices])
            if mode_selection in ("Crossline", "Both"):
                target_keys.extend([("Crossline", idx) for idx in xline_indices])

            if not target_keys:
                self.extract_status.object = "No slices found for extraction settings."
                return

            self.extracted_slices = {}
            self.extracted_keys_by_mode = {"Inline": [], "Crossline": []}
            self.slice_result_cache = {}
            self.active_slice_key = None

            total = len(target_keys)
            for position, (mode, idx) in enumerate(target_keys, start=1):
                self.extract_status.object = f"Extracting {position}/{total} from {mode.lower()} {idx}..."
                data, x_dim, y_dim = self._prepare_visual_slice(mode, idx)
                key = (mode, int(idx))
                self.extracted_slices[key] = data
                self.slice_result_cache[key] = {
                    "locked_meta": {"mode": mode, "idx": int(idx), "xl": x_dim, "yl": y_dim},
                    "res_mag": None,
                    "res_ori": None,
                    "res_vec": None,
                    "flowlines": [],
                    "flowline_ids": np.array([], dtype=int),
                    "seismic_cluster_map": None,
                    "unconformity_surfaces": [],
                    "cluster_labels": None,
                }
                self.extracted_keys_by_mode[mode].append(int(idx))
                progress_value = 10 + int(80 * position / max(1, total))
                self.extract_progress.value = progress_value
                self._report_progress("Extraction", progress_value)

            self.extracted_keys_by_mode["Inline"].sort()
            self.extracted_keys_by_mode["Crossline"].sort()

            mode_options = []
            if self.extracted_keys_by_mode["Inline"]:
                mode_options.append("Inline")
            if self.extracted_keys_by_mode["Crossline"]:
                mode_options.append("Crossline")
            self.view_mode_group.options = mode_options
            self.view_mode_group.disabled = len(mode_options) <= 1
            self.view_mode_group.value = mode_options[0]
            self._refresh_view_slice_slider()
            ...
            n_inline = len(self.extracted_keys_by_mode["Inline"])
            n_xline = len(self.extracted_keys_by_mode["Crossline"])
            self.extract_status.object = f"Extracted {n_inline} inline and {n_xline} crossline slices. Select one to process."
            self.logger.info(f"[Extraction] 2.5D extracted | inline={n_inline} crossline={n_xline} step={nth}")
    ...
    def run_gst_engine(self, event) -> None:
        if not self.extracted_slices:
            return

        self.gst_calc_btn.disabled = True
        self.gst_progress.visible = True
        self.gst_progress.value = 10
        self.gst_status.object = "Computing GST for extracted slices..."
        self._report_progress("GST", 10)
        self.logger.info(f"[GST] Starting 2.5D batch | device={self.device.type}")
        ...
        def worker() -> None:
            try:
                keys = self._ordered_extracted_keys()
                ...
                for index, key in enumerate(keys, start=1):
                    mode, idx = key
                    data = self.extracted_slices.get(key)
                    if data is None:
                        continue

                    ui_call(lambda m=mode, i=idx, n=index, t=total: setattr(
                        self.gst_status,
                        "object",
                        f"Computing GST {n}/{t}: {m} {i}",
                    ))

                    def gst_progress(value: int, n=index, t=total) -> None:
                        mapped = int(((n - 1) * 100 + value) / max(1, t))
                        ui_call(lambda val=mapped: setattr(self.gst_progress, "value", val))
                        self._report_progress("GST", mapped)
                    ...
                    cache_item = self.slice_result_cache.get(key, {})
                    cache_item["res_mag"] = mag
                    cache_item["res_ori"] = ori
                    cache_item["res_vec"] = vec
                    cache_item["flowlines"] = []
                    cache_item["flowline_ids"] = np.array([], dtype=int)
                    cache_item["seismic_cluster_map"] = None
                    cache_item["unconformity_surfaces"] = []
                    cache_item["cluster_labels"] = None
                    self.slice_result_cache[key] = cache_item

                def on_done():
                    current_mode = str(self.view_mode_group.value)
                    current_idx = int(self.view_slice_slider.value) if not self.view_slice_slider.disabled else None
                    if current_idx is not None:
                        self._set_active_slice(current_mode, current_idx)
                    self.gst_progress.value = 100
                    self.gst_status.object = "GST complete for all extracted slices."
                    self.fl_calc_btn.disabled = False
                    self.exp_btn.disabled = False
                    self.update_view()
                    self._save_state_to_cache()
                    self._report_progress("GST", 100)
                    self.logger.info(f"[GST] Completed | n_slices={len(keys)}")
    ...
    def run_flowline_engine(self, event) -> None:
        ...
        def worker() -> None:
            try:
                ...
                for slice_pos, key in enumerate(keys, start=1):
                    mode_name, idx_value = key
                    cache_item = self.slice_result_cache.get(key, {})
                    slice_data = self.extracted_slices.get(key)
                    slice_ori = cache_item.get("res_ori")
                    slice_vec = cache_item.get("res_vec")
                    if slice_data is None or slice_ori is None:
                        continue

                    ui_call(lambda m=mode_name, i=idx_value, n=slice_pos, t=total_slices: setattr(
                        self.fl_count_label,
                        "object",
                        f"Extracting flowlines {n}/{t}: {m} {i}",
                    ))

                    h, w = slice_data.shape
                    vector_array = slice_vec
                    if vector_array is None:
                        vec_x = -np.sin(slice_ori)
                        vec_y = np.cos(slice_ori)
                        flip_mask = vec_x < 0
                        vec_x = np.where(flip_mask, -vec_x, vec_x)
                        vec_y = np.where(flip_mask, -vec_y, vec_y)
                    ...
                    for col_index, x_seed in enumerate(seed_columns, start=1):
                        ...
                        if col_index % progress_stride == 0 or col_index == total_columns:
                            local_part = col_index / max(1, total_columns)
                            mapped = int(((slice_pos - 1 + local_part) / max(1, total_slices)) * 100)
                            ui_call(lambda val=mapped: setattr(self.fl_progress, "value", val))
                            self._report_progress("Flowlines", mapped)

                    cache_item["res_vec"] = vector_array
                    cache_item["flowlines"] = lines
                    cache_item["flowline_ids"] = np.arange(1, len(lines) + 1, dtype=int)
                    cache_item["cluster_labels"] = None
                    cache_item["seismic_cluster_map"] = None
                    cache_item["unconformity_surfaces"] = []
                    self.slice_result_cache[key] = cache_item
                ...
                def on_done():
                    ...
                    if current_idx is not None:
                        self._set_active_slice(current_mode, current_idx)
                    n_with_flowlines = sum(1 for key in keys if len(self.slice_result_cache.get(key, {}).get("flowlines", [])) > 0)
                    self.fl_progress.value = 100
                    self.fl_apply_btn.disabled = n_with_flowlines == 0
                    self.cl_calc_btn.disabled = n_with_flowlines == 0
                    self.sc_calc_btn.disabled = n_with_flowlines == 0
                    self.uc_calc_btn.disabled = n_with_flowlines == 0
                    self.uc_apply_btn.disabled = True
                    self.fl_count_label.object = f"Flowlines extracted for {n_with_flowlines}/{len(keys)} slices"
                    self.update_view()
                    self._save_state_to_cache()
                    self.logger.info(f"[Flowlines] Completed | n_slices={len(keys)} with_flowlines={n_with_flowlines}")
