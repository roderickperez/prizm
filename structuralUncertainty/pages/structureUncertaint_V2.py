...
def _set_petrel_export_status(message: str) -> None:
    ...
    if message.startswith("❌"):
        logger.error("Petrel export status: %s", message)
    elif message.startswith("⚠️"):
        logger.warning("Petrel export status: %s", message)
    else:
        logger.info("Petrel export status: %s", message)


def _safe_value_repr(value) -> str:
    try:
        text = repr(value)
    except Exception:
        text = "<unreprable>"
...
def _log_widget_change(event, label: str) -> None:
    try:
        logger.info(
            "UI change | %s | old=%s | new=%s",
            label,
            _safe_value_repr(getattr(event, "old", None)),
            _safe_value_repr(getattr(event, "new", None)),
        )
    except Exception:
        logger.exception("Failed to log widget change for %s", label)


for _widget, _label in [
    (sel_surface, "Select Surface"),
    (sld_n_maps, "Number of Velocity Realizations"),
    (sld_std_dev, "Velocity Std. Dev."),
    (sld_smooth, "Smoothing Sigma"),
    (sld_section_angle, "Section/Cut Angle"),
    (sld_c_inc, "Contour Search Step"),
    (sld_depth_contours, "Depth Contour Range"),
    (chk_eliminate, "Eliminate outside closure"),
    (chk_close_poly, "Show closure contour"),
    (chk_depth_contour, "Contour Final Depth Map"),
    (sld_slope, "Variogram Model Type"),
    (sld_range, "Variogram Range"),
    (sld_sill, "Variogram Sill"),
    (sld_nugget, "Variogram Nugget"),
    (sld_thick_mean, "Thickness Mean"),
    (sld_thick_std, "Thickness Std Dev"),
    (sld_ntg, "Net-to-Gross Ratio"),
    (sld_poro, "Porosity"),
    (sld_sw, "Water Saturation"),
    (sld_fvf, "Formation Volume Factor"),
    (rg_velocity_units, "Velocity Units"),
    (rg_xy_units, "XY Units"),
    (rg_depth_units, "Depth Units"),
    (rg_time_units, "Time Units"),
    (rg_area_units, "Area Units"),
    (rg_volume_units, "Volume Units"),
    (sel_report_type, "Report Type"),
    (txt_report_filename, "Report Filename"),
    (txt_report_location, "Report Location"),
    (txt_export_petrel_surface_name, "Petrel Export Prefix"),
    (chk_export_average_mean, "Export Average Mean"),
    (chk_export_final_depth, "Export Final Depth"),
    (chk_export_isoprobability, "Export Isoprobability"),
    (chk_export_all_realizations, "Export All Realizations"),
]:
    _widget.param.watch(lambda event, _label=_label: _log_widget_change(event, _label), "value")

for _button, _label in [
    (btn_generate_surf, "Generate / Update TWT Input"),
    (btn_update_depth_maps, "Update Velocity & Depth Realizations"),
    (btn_update_vario, "Update Variogram"),
    (btn_update_culm, "Update Closure / Culmination"),
    (btn_update_volumetrics, "Run Monte Carlo Volumetrics"),
    (btn_export_report, "Generate Report"),
    (btn_export_petrel_surface, "Export Selected Surfaces to Petrel"),
]:
    _button.param.watch(lambda event, _label=_label: logger.info("UI click | %s | clicks=%s", _label, _safe_value_repr(getattr(event, "new", None))), "clicks")
